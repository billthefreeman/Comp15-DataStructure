...
bool Maze::solve_maze_recursive(int row, int col) {
	...
	if (row< 0 || row>= maze_array_size ||col<0||col>=maze_array_size ) {
		return false;
	} else if (maze[row][col] == 'X' || maze[row][col] == 'b'
			|| maze[row][col] == '.') {
		return false;
	} else if (maze[row][col] == 'F')
		return true;
	maze[row][col] = '.';//Mark maze[row][col] as a possible path
...
void Maze::solve_maze_stack(int row, int col) {
	...
	while (1) {
		...
		//west
		if (col - 1 >= 0 && maze[row][col - 1] != 'X' && maze[row][col
				- 1] != 'b' && maze[row][col - 1] != '.') {
			stack.push(Position(row, col));
			col -= 1;
			continue;
		...
		maze[row][col] = 'b';//we’1ve reached a dead end
		Position pos = stack.pop();//backtrack by popping the stack

		row = pos.row;
		col = pos.col;

		//setting row and col to the position just popped
	}
//		maze[row][col] = 'b';//we’1ve reached a dead end
//		Position pos = stack.pop();//backtrack by popping the stack
...
void Maze::maze_generator(int index, int backtrack_x[CELL],
	...
	if (visited < n * n) {
		...
		maze_generator(index, backtrack_x, backtrack_y, x_next, y_next,
				n, visited);
	}
}
...
int Maze::is_closed(int x, int y) {
	if (maze[x - 1][y] == WALL && maze[x][y - 1] == WALL && maze[x][y + 1]
			== WALL && maze[x + 1][y] == WALL)
		return 1;
