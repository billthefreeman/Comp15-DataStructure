...
int IsisCourse::waitlist_position(ENROLLMENT_STATUS status, Student s) {
	// TODO: Student writes code here
	Queue *waitlist_queue = NULL;
	Queue temp_queue;
	int count = 0;
	bool found = false;
	assert(status == MAJOR_WAITLIST ||status == OTHER_WAITLIST);
	...
	while (!waitlist_queue->is_empty()) {
		count++;
		Student n = waitlist_queue->dequeue();
		temp_queue.enqueue(n);
		if (n.name == s.name) {
			found = true;
			break;
		}
...
IsisCourse::ENROLLMENT_STATUS IsisCourse::status(Student s) {
	...
	if (s.major && waitlist_position(MAJOR_WAITLIST, s) > 0)
		return status = MAJOR_WAITLIST;
	if (!s.major && waitlist_position(OTHER_WAITLIST, s) > 0)
...
void IsisCourse::update_enrollments() {
	...
	while (!major_waitlist.is_empty() && roster.size() < class_capacity)
		roster.add(major_waitlist.dequeue());
	while (!other_waitlist.is_empty() && roster.size() < class_capacity)
